...
class RestService:
    ...
    async def set_status(self, request, criteria=None):
        ...
        report_dict, error = await self._report_pre_check(
    ...
    async def update_report_dates(self, request, criteria=None):
        ...
        if (not apply_to_all) and (start_date or end_date):
            ...
            if start_date and end_date:
                date_query = "({a} OR {b} OR {c} OR {d})".format(
                    a=start_date_lt, b=end_date_gt, c=start_date_gt, d=end_date_lt
                )
            ...
            bounds_query = (
                "SELECT * FROM report_sentence_hits WHERE " + date_query + " AND report_uid = "
            ...
            if out_of_bounds:
                ...
                error_msg = (
                    ("%s confirmed technique" % number)
                    + (" has" if number == 1 else "s have")
        ...
        if model_initially_predicted:
            ...
            if not existing:  # Only add to the true positives table if it's not already there
                sql_commands.append(
                    await self.dao.insert_generate_uid(
                        "true_positives",
                        dict(sentence_id=sen_id, attack_uid=attack_id, true_positive=sentence_to_insert),
                        return_sql=True,
                    )
                )
        else:
            ...
            if not existing:  # Only add to the false negatives table if it's not already there
                sql_commands.append(
                    await self.dao.insert_generate_uid(
                        "false_negatives",
                        dict(sentence_id=sen_id, attack_uid=attack_id, false_negative=sentence_to_insert),
                        return_sql=True,
                    )
                )
        ...
        if model_initially_predicted:
            ...
            if not existing:  # Only add to the false positives table if it's not already there
                sql_commands.append(
                    await self.dao.insert_generate_uid(
                        "false_positives",
                        dict(sentence_id=sen_id, attack_uid=attack_id, false_positive=sentence_to_insert),
                        return_sql=True,
                    )
                )
    ...
    async def update_attack_time(self, request, criteria=None):
        ...
        for mapping in mapping_list:
            ...
            inv_start = (
                start_date_conv
                and (not end_date_conv)
                and current_end
                and (start_date_conv > current_end.replace(tzinfo=None))
            )
            inv_end = (
                end_date_conv
                and (not start_date_conv)
                and current_start
                and (end_date_conv < current_start.replace(tzinfo=None))
            )
        ...
        if updates:
            ...
            if r_update_data:
                updates.append(
                    await self.dao.update("reports", where=dict(uid=report_id), data=r_update_data, return_sql=True)
                )
    ...
    async def check_edit_sentence_permission(
        self, request, criteria=None, default_error=None, action="unspecified", strict=False
    ):
