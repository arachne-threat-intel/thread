...
class IoCManager(ReportEntityManager):
    ...
    async def suggest_ioc(self, request, criteria=None):
        ...
        default_error = {"error": "Error predicting IoC."}
        sentence_data, _report_id, error = await self.check_edit_sentence_permission(
        ...
        _is_public_ip, cleaned_ioc_text = check_if_public_ip(cleaned_ioc_text, clean=True)
    ...
    async def update_ioc(self, request, criteria=None, adding=False, deleting=False):
        ...
        default_error, success = {"error": "Error updating IoC."}, REST_SUCCESS.copy()
        ...
        db_query = {"report_id": report_id, "sentence_id": sentence_data[UID]}

        if deleting:
            await self.dao.delete(table, db_query)
            success.update({"info": "The selected sentence is no longer flagged as an IoC.", "alert_user": 1})
            return success
        ...
        is_public_ip, _cleaned_ip = check_if_public_ip(text, clean=False)
        ...
        if existing and not adding:
            if existing[0]["refanged_sentence_text"] == text:
                return REST_IGNORED
            await self.dao.update(table, where=db_query, data={"refanged_sentence_text": text})
            success.update({"info": "This sentence-IoC text has been updated.", "alert_user": 1})
        else:
            await self.dao.insert_generate_uid(table, dict(**db_query, refanged_sentence_text=text))
            success.update({"info": "The selected sentence has been flagged as an IoC.", "alert_user": 1})
    ...
    def __refang(self, ioc_text):
        ...
        replace_periods = "[{}]+".format(re.escape("".join(self.web_svc.PERIODS)))
        ...
        remove_quotes = "[{}]+".format(re.escape("".join(set(self.web_svc.QUOTES) - {"''"})))
        ioc_text = re.sub(remove_quotes, "", ioc_text)
        ioc_text = re.sub("({})+".format(re.escape("''")), "", ioc_text)
        ...
        replace_start_pattern = "^[{}]+".format(re.escape("".join(replace_start)))
        replace_end_pattern = "[{}]+$".format(re.escape("".join(replace_end)))
