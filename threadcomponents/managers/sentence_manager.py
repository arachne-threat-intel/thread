...
class SentenceManager(ReportEntityManager):
    ...
    async def sentence_context(self, request, criteria=None):
        ...
        ioc = await self.dao.get("report_sentence_indicators_of_compromise", {"sentence_id": sen_id})
        ...
        return {"techniques": techniques, "ioc": ioc}
    ...
    async def remove_sentence(self, request, criteria=None):
        ...
        default_error = {"error": "Error removing item."}
        ...
        await self.dao.delete("report_sentences", {"uid": sen_id})
        # This could also be an image, so delete from original_html table too
        await self.dao.delete("original_html", {"uid": sen_id})
