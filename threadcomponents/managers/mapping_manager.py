...
from threadcomponents.helpers.date import pre_save_date_checks, to_datetime_obj
...
class MappingManager(ReportEntityManager):
    ...
    async def reject_attack(self, request, criteria):
        ...
        try:
            ...
            return {"error": "Error rejecting attack."}

        logger.info(f"Rejecting attack {attack_id} for sentence {sen_id}")

        sentence_dict = await self.dao.get("report_sentences", {"uid": sen_id})
        attack_dict = await self.dao.get("attack_uids", {"uid": attack_id})

        # Check the method can continue
        checks, _report = await self.check_edit_mapping_permission(
    ...
    async def ignore_attack(self, request, criteria):
        ...
        try:
            ...
            logger.warning("`sentence_id` or `attack_uid` not found in request data")
            return {"error": "Error rejecting attack."}

        logger.info(f"Ignoring attack {attack_id} for sentence {sen_id}")
    ...
    async def add_attack(self, request, criteria):
        ...
        try:
            ...
            return {"error": "Error adding attack."}

        logger.info(f"Accepting attack {attack_id} for sentence {sen_id}")
        ...
        historic_hits = await self.dao.get("report_sentence_hits", {"sentence_id": sen_id, "attack_uid": attack_id})
    ...
    async def update_attack_time(self, request, criteria=None):
        ...
        default_error, success = {"error": "Error updating technique times."}, REST_SUCCESS.copy()
        ...
        if not start_date:
            return {"error": "Technique Start Date missing.", "alert_user": 1}

        if (not mapping_list) or (not isinstance(mapping_list, list)):
            return {"error": "No Confirmed Techniques selected.", "alert_user": 1}
